...
export async function POST(request: NextRequest) {
  try {
    ...
    const points_earned = action[0].points * quantity

    // Create user action with pending status
    const result = await sql`
      INSERT INTO user_actions (user_id, action_id, quantity, points_earned, description, photo_url, status)
      VALUES (${user_id}, ${action_id}, ${quantity}, ${points_earned}, ${description}, ${photo_url}, 'pending')
      RETURNING *
    `
